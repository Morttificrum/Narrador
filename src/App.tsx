...
export default function App() {
  ...
  const speakBlock = (block: ScriptBlock) => {
    if (!audioCtxRef.current) return;
    ...
    // Configura a URL apontando para a nossa API proxy de TTS com voz e emoção do Gemini
    audio.src = `/api/tts?text=${encodeURIComponent(textToSpeak)}&voice=${geminiVoice}&emotion=${encodeURIComponent(geminiEmotion)}`;
    ...
    audio.onended = () => {
      ...
      // Verifica se há o próximo na fila
    ...
    audio.onerror = (e) => {
      ...
      // Pula para o próximo bloco se houver
      if (blockQueueRef.current.length > 0) {
        const nextBlock = blocks.find(b => b.id === blockQueueRef.current[0]);
        if (nextBlock) speakBlock(nextBlock);
      }
